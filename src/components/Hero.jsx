...
export default function Hero({ profile }) {
  ...
  // Build full URL for images/resume so they work in local dev (Vite doesn't proxy /uploads)
  const imgSrc = profile?.profileImage
    ? (profile.profileImage.startsWith('http') ? profile.profileImage : `${BASE_URL}${profile.profileImage}`)
    : null;

  const resumeURL = profile?.resume ? `${BASE_URL}/api/profile/download-resume` : null;
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:60, alignItems:'center' }}>
          ...
          <div style={{ animation:'fadeUp 0.8s ease both' }}>
            ...
            <div style={{ display:'flex', gap:16, flexWrap:'wrap',
              ...
              {resumeURL && (
                <a href={resumeURL} download="resume.pdf" target="_blank" rel="noreferrer" className="btn btn-ghost">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
