  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex' }}>
      ...
      <main style={{ marginLeft:240, flex:1, padding:'40px 40px', maxWidth:900, position:'relative', zIndex:1 }}>
        ...
        {/* ── SKILLS TAB ─────────────────────────────────────────────────── */}
        {tab === 'skills' && (
          <Panel title="Edit Skills" subtitle="Type a skill and press Enter or click Add. Click × to remove.">
            {[
              ['frontend','Frontend','🎨'],['backend','Backend','⚙️'],
              ['database','Database','🗄️'],['features','Backend Features','🛡️'],['tools','Tools','🔧']
            ].map(([k, label, icon]) => (
              <SkillEditor
                key={k}
                label={label}
                icon={icon}
                skills={profile.skills?.[k] || []}
                onChange={arr => setProfile({ ...profile, skills: { ...profile.skills, [k]: arr } })}
              />
            ))}
            <button className="btn btn-primary" onClick={saveProfile} disabled={saving}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
              ...
              <div className="card" style={{ padding:'28px', textAlign:'center' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  ...
                  {profile.resume && (
                    <a
                      href={`${BASE_URL}/api/profile/download-resume`}
                      download="resume.pdf"
                      target="_blank"
                      rel="noreferrer"
...
// ── Skill Editor ──────────────────────────────────────────────────────────────
function SkillEditor({ label, icon, skills, onChange }) {
  ...
  return (
    <div className="card" style={{ marginBottom: 16, padding: '22px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-code)', fontSize: '0.72rem',
          color: 'var(--text-muted)', background: 'var(--bg)', padding: '2px 8px', borderRadius: 99 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <input
          ...
          onKeyDown={handleKey}
          placeholder={`Add a ${label} skill and press Enter...`}
        />
        <button
          type="button"
      ...
      {skills.length > 0 ? (
        ...
        <div style={{ fontFamily: 'var(--font-code)', fontSize: '0.78rem',
          color: 'var(--text-muted)', textAlign: 'center', padding: '12px 0' }}>
